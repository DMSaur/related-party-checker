...
你需要基于证据进行合理推断，如果证据显示两家公司名称高度相关（如包含相同的品牌名/母公司名），且有迹象表明存在投资或控制关系，可以判定为关联。"""

USER_PROMPT_TEMPLATE = """【证据文本】
{evidence}

【判断任务】
根据证据文本，判断以下两家公司是否存在关联交易关系:
...
6. 公司名称包含相同的品牌/集团名称，且证据显示存在投资或控制关系

判断规则:
- 如果两家公司名称包含相同的品牌名(如Samsung、BYD、Goertek等)，且证据提到其中一家是另一家的子公司、关联公司、或在该品牌投资背景下设立，应判定为关联
- 如果证据明确显示越南公司是中国公司的海外生产基地、子公司、或FDI项目，判定为关联
- 如果证据不足且无任何关联迹象，才输出"无法判断"
